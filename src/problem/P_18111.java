...
public class P_18111 implements Problem {
    ...
    public void exec() throws Exception {
        ...
        for (int h = min; h <= max; h++) { // 땅의 높이 후보
            ...
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < M; j++) {
                    ...
                    if (diff > 0) {
                        time += diff * 2;
                        blockCount += diff; // 블록을 제거하여 인벤토리에 추가
                    } else if (diff < 0) {
                        time += -diff;
                        blockCount += diff; // 블록을 쌓아서 필요한 블록을 차감
                    }
                }
            }

            if (blockCount >= 0) { // 인벤토리에 충분한 블록이 있을 때
                if (time <= minTime) { // 시간이 최소 시간보다 작거나 같을 때
                    minTime = time;
                    maxHeight = h;
                }
            }
