...
public class P_10798 implements Problem {
    ...
    public void exec() throws IOException {
        ...
        String[][] wordMatrix = new String[5][15];

        for (int row=0; row<5; row++) {
            for (int column=0; column<15; column++)
                wordMatrix[row][column] = "-";
        }
        ...
        for (int row=0; row<5; row++) {
            ...
            for (int column=0; column<stringLength; column++) {
                wordMatrix[row][column] = String.valueOf(inputString.charAt(column));
            }
        }

        for (int column=0; column<15; column++) {
            for (int row=0; row<5; row++)
                if (!wordMatrix[row][column].equals("-"))
